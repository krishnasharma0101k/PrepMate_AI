      ...
      <div className={`px-3 h-[52px] border-b border-black/8 shrink-0 flex items-center ${SidebarOpen ? "justify-between" : "justify-center"}`}> 
        {SidebarOpen && (
          <div className='flex items-center gap-2.5'>
            ...
            className='font-extrabold text-sm tracking-tight text-[#0A0A0A] whitespace-nowrap'>PrepMate AI</motion.span>
          </div>
        )}
       ...
       <div className='px-2.5 pt-3 pb-1.5 shrink-0'>
        <motion.button
        onClick = {onNewInerview}
        whileHover = {{scale: 1.03}}
        whileTap = {{scale: 0.97}}
        className={`w-full flex items-center gap-2 bg-[#000000] text-white font-semibold rounded-lg py-2 transition-all shadow-[0_4px_14px_rgba(0,0,0,0.25)] cursor-pointer hover:bg-[#lalala] ${SidebarOpen ? "px-2.5" : "justify-center px-0"}`}>
       ...
       <nav className='flex flex-col gap-0.5 px-2.5 flex-1'>
        ...
        <AnimatePresence> 
          ...
          className='group flex cursor-opinter items-center justify-between gap-2.5 rounded-lg border border-white/10 bg-[#000000]/90 blackdrop-blur-2xl px-2.5 py-1.5 mb-2.5 transition-all hover:border-white/20 shadow-[0_8px_24px_rgba(0,0,0,0.2)]'>
